...
std::string MilManager::diagnostics_NoLock() const
{
    ...
    bool MilManager::grabToRGBA8(int deviceNum,
        ...
        int& outWidth, int& outHeight)
    {
        ...
        {
            _lastError = "Digitizer not allocated for device " + std::to_string(deviceNum);
            return false;
        }
        ...
        // Blocking single-frame grab into d.grabBuf
        ...
        // Temporary per-band buffers (you can cache these on Dig for speed later)
        std::vector<uint8_t> b0((size_t)w * (size_t)h);
        std::vector<uint8_t> b1, b2;

        if (d.bands >= 3)
        {
            b1.resize((size_t)w * (size_t)h);
            b2.resize((size_t)w * (size_t)h);

            // Pull bands from MIL into CPU buffers
            // (Your project already has bufGetColor2d helper using MbufChildColor + MbufGet2d)
